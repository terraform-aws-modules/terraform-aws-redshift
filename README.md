...
|------|-------------|:----:|:-----:|:-----:|
| allow\_version\_upgrade | (Optional) If true, major version upgrades can be applied during the maintenance window to the Amazon Redshift engine that is running on the cluster. | string | `"true"` | no |
| automated\_snapshot\_retention\_period | How long will we retain backups | string | `"0"` | no |
| cluster\_database\_name | The name of the database to create | string | n/a | yes |
| cluster\_iam\_roles | A list of IAM Role ARNs to associate with the cluster. A Maximum of 10 can be associated to the cluster at any time. | list | `[]` | no |
| cluster\_identifier | Custom name of the cluster | string | n/a | yes |
| cluster\_master\_password |  | string | n/a | yes |
| cluster\_master\_username | Self-explainatory variables | string | n/a | yes |
| cluster\_node\_type | Node Type of Redshift cluster | string | n/a | yes |
| cluster\_number\_of\_nodes | Number of nodes in the cluster (values greater than 1 will trigger 'cluster_type' of 'multi-node') | string | `"3"` | no |
| cluster\_parameter\_group | Parameter group, depends on DB engine used | string | `"redshift-1.0"` | no |
| cluster\_port |  | string | `"5439"` | no |
| cluster\_version | Version of Redshift engine cluster | string | `"1.0"` | no |
| enable\_logging | Enables logging information such as queries and connection attempts, for the specified Amazon Redshift cluster. | string | `"false"` | no |
...
| enhanced\_vpc\_routing | (Optional) If true, enhanced VPC routing is enabled. | string | `"false"` | no |
| final\_snapshot\_identifier | (Optional) The identifier of the final snapshot that is to be created immediately before deleting the cluster. If this parameter is provided, 'skip_final_snapshot' must be false. | string | `"false"` | no |
...
| logging\_bucket\_name | (Optional, required when enable_logging is true) The name of an existing S3 bucket where the log files are to be stored. Must be in the same region as the cluster and the cluster must have read bucket and put object permissions. | string | `"false"` | no |
| logging\_s3\_key\_prefix | (Optional) The prefix applied to the log file names. | string | `"false"` | no |
...
| publicly\_accessible | Determines if Cluster can be publicly available (NOT recommended) | string | `"false"` | no |
...
| vpc\_security\_group\_ids | A list of Virtual Private Cloud (VPC) security groups to be associated with the cluster. | list | n/a | yes |
